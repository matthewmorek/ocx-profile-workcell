...
const validEvidence = () => ({ schemaVersion: 1, version: "0.1.0", commit, installedComponents: ["workspace", "ws", "ws-overrides"], resolvedDependencies: {}, assertions: { install: true }, receipt: { component: "ws" }, toolVersions: { ocx: "2.0.14", opencode: "1.17.15" } });
...
async function writeOcxOutput(root: string, packuments: Record<string, unknown>): Promise<void> {
  ...
  await Bun.write(join(root, "index.json"), JSON.stringify({ version: "1.0.0" }));
...
function tarEntry(path: string, content: string, type = "0", mode = 0o644): Buffer {
  const header = Buffer.alloc(512); header.write(path); octal(mode, 8).copy(header, 100); octal(content.length, 12).copy(header, 124); octal(0, 12).copy(header, 136); header.fill(0x20, 148, 156); header.write(type, 156); header.write("ustar\0", 257); header.write("00", 263);
...
function archive(...entries: Buffer[]): Uint8Array { return gzipSync(Buffer.concat([...entries, Buffer.alloc(1024)]), { mtime: 0 }); }
...
describe("strict CLI and evidence boundaries", () => {
  ...
  test("rejects incomplete, failed, path-contaminated, and secret-like evidence", () => {
    ...
    expect(() => parseInstallEvidence({ ...validEvidence(), installedComponents: ["ws"] })).toThrow("component set");
    expect(() => parseInstallEvidence({ ...validEvidence(), assertions: { install: false } })).toThrow("failed assertions");
    expect(() => parseInstallEvidence({ ...validEvidence(), receipt: { path: "/Users/example/.config" } })).toThrow("machine path");
    expect(() => parseInstallEvidence({ ...validEvidence(), receipt: { apiKey: "nope" } })).toThrow("secret-like");
...
describe("OCX output normalization", () => {
  test("requires exactly two packuments and fully rewrites their version contract", async () => {
    ...
    try {
      ...
      await Bun.write(join(root, "components/extra.json"), JSON.stringify(packument()));
...
describe("deterministic artifact contracts", () => {
  ...
  test("packages complete release assets in stable digest order", async () => {
    ...
    try {
      ...
      await runScript("scripts/verify-release.ts", ["bundle", "--archive", join(root, "one", archive), "--provenance", join(root, "one", "provenance.json"), "--receipt", join(root, "one", "receipt.jsonc"), "--checksums", join(root, "one", "SHA256SUMS"), "--extract-to", join(root, "extracted")]);
    } finally { await rm(root, { recursive: true, force: true }); }
  });
