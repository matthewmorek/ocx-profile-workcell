...
async function main(): Promise<void> {
  ...
  await writeJsonAtomic(requiredArgument(arguments_, "--out"), { schemaVersion: 1, tag, version, commit, archiveSha256: await sha256File(requiredArgument(arguments_, "--archive")), evidence, files: await Promise.all(files.map(async (path) => ({ path, sha256: await sha256File(join(pages, path)), mode: (await stat(join(pages, path))).mode & 0o777 }))) });
}
