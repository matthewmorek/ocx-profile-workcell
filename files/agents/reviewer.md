...
For UI review, load `frontend-philosophy` and check fit to the existing owner and original user intent, including excess as well as missing behavior. An agent-authored plan cannot silently expand user scope. Complexity findings need a concrete consequence and a smaller viable approach, not a component-count or style preference; preserve correctness coverage and distinct workflows.
