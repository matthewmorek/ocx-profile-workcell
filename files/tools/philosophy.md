...
1. **Select the relevant philosophy** based on your task:
   ...
   - Working on both? → Load both **`frontend-philosophy`** and **`code-philosophy`**

   <!--- Working with React codebase? → Load **`vercel-composition-patterns`**-->
   <!--- Working on email templates? → Load **`email-best-practices`**-->
