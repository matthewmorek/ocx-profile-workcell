 ...
 * @param projectRoot - Absolute path to the project root
 */
async function getDbPath(projectRoot: string, databaseDirectory = getDbDirectory()): Promise<string> {
	const projectId = await getProjectId(projectRoot)
	return path.join(databaseDirectory, `${projectId}.sqlite`)
}
...
export async function initStateDb(
	...
	if (options.databaseDirectory !== undefined && !options.databaseDirectory.trim()) {
		throw new Error("initStateDb databaseDirectory must not be empty")
	}

	const dbPath = await getDbPath(projectRoot, options.databaseDirectory)
	...
	try {
		// Bound lock contention before any write-capable schema operation.
		...
		migrateLegacyPendingDeletes(db)
		return db
	} catch (error) {
		db.close()
		throw error
...
function migrateLegacyPendingDeletes(db: Database): void {
	const migrate = db.transaction(() => {
		// Preserve a legacy singleton only when its owning session is unambiguous.
		db.exec(`
		INSERT OR IGNORE INTO pending_deletes (session_id, branch, path)
		SELECT sessions.id, pending_operations.branch, pending_operations.path
		FROM pending_operations
		JOIN sessions
			ON sessions.branch = pending_operations.branch
			AND sessions.path = pending_operations.path
		WHERE pending_operations.type = 'delete'
			AND (
				SELECT COUNT(*)
				FROM sessions AS candidates
				WHERE candidates.branch = pending_operations.branch
					AND candidates.path = pending_operations.path
			) = 1
		`)

		// Ambiguous legacy deletes cannot be safely attributed to an idle session.
		db.exec(`DELETE FROM pending_operations WHERE type = 'delete'`)
	})

	// Acquire the write reservation before reading legacy rows so another writer
	// cannot insert between the copy and delete statements.
	migrate.immediate()
}
...
/** Remove exactly one worktree session after its worktree is removed. */
...
/**
 * Set a pending delete operation scoped to the requesting session.
 * A repeated request from the same session replaces only that session's row.
 *
 * @param db - Database instance from initStateDb
...
export function setPendingDelete(db: Database, del: PendingDelete, client?: OpencodeClient): void {
	...
	// Atomic: replace only the requesting session's pending delete.
	const stmt = db.prepare(`
		INSERT OR REPLACE INTO pending_deletes (session_id, branch, path)
		VALUES ($sessionId, $branch, $path)
	`)
...
/**
 * Get the pending delete operation for one requesting session.
 *
 * @param db - Database instance from initStateDb
...
export function getPendingDelete(db: Database, sessionId: string): PendingDelete | null {
	...
	const stmt = db.prepare(`
		SELECT session_id as sessionId, branch, path
		FROM pending_deletes
		WHERE session_id = $sessionId
	`)

	const row = stmt.get({ $sessionId: sessionId }) as Record<string, string> | null
	if (!row) return null
...
/**
 * Clear the pending delete for one requesting session.
 *
 * @param db - Database instance from initStateDb
 */
export function clearPendingDelete(db: Database, sessionId: string): void {
	...
	const stmt = db.prepare(`DELETE FROM pending_deletes WHERE session_id = $sessionId`)
