 ...
 *
 * Inspired by Felix Anhalt's opencode-worktree-session v1.1.0 at
 * 93a55c23c9fd5ce9328d090d31a74e7357af5d8d (Apache-2.0).
 * No file-level copying from that project is asserted.
 *
...
import { access, copyFile, cp, mkdir, readFile, realpath, rm, stat, symlink } from "node:fs/promises"
...
/** Preserve Git's NUL-delimited output exactly for path-bearing porcelain commands. */
async function gitRaw(args: string[], cwd: string): Promise<Result<Uint8Array, string>> {
	try {
		const proc = Bun.spawn(["git", ...args], {
			cwd,
			stdout: "pipe",
		...
		return Result.ok(new Uint8Array(stdout))
	} catch (error) {
		return Result.err(error instanceof Error ? error.message : String(error))
	}
}
...
type WorktreeDeleteOutcome =
	| { status: "none" }
	| { status: "removed"; branch: string; path: string }
	| { status: "reconciled"; branch: string; path: string }
	| { status: "retained"; reason: string }

type ClearDeleteState = (database: Database, sessionID: string) => void
type BeforeSnapshot = (worktreePath: string) => Promise<void>
type PrepareDelete = () => Promise<BeforeSnapshot | undefined>
type GitRaw = (args: string[], cwd: string) => Promise<Result<Uint8Array, string>>

interface ProcessPendingWorktreeDeleteOptions {
	database: Database
	sessionID: string
	repoRoot: string
	log: Logger
	...
	prepareDeleteFn: PrepareDelete
	pathExistsFn?: typeof pathExists
	clearDeleteStateFn?: ClearDeleteState
}

function clearDeleteState(database: Database, sessionID: string): void {
	const clearRemovedState = database.transaction(() => {
		clearPendingDelete(database, sessionID)
		removeSessionById(database, sessionID)
	})
	clearRemovedState.immediate()
}

const textDecoder = new TextDecoder("utf-8", { fatal: true })

function decodeGitField(field: Uint8Array): Result<string, string> {
	try {
		return Result.ok(textDecoder.decode(field))
	} catch {
		return Result.err("git worktree porcelain contains a path that is not valid UTF-8")
	}
}

function parseGitWorktreePaths(worktreeList: Uint8Array): Result<string[], string> {
	if (worktreeList.length === 0) {
		return Result.err("git worktree porcelain was empty")
	}

	const records: Uint8Array[][] = []
	let fields: Uint8Array[] = []
	let fieldStart = 0
	for (let index = 0; index < worktreeList.length; index++) {
		if (worktreeList[index] !== 0) continue

		const field = worktreeList.subarray(fieldStart, index)
		fieldStart = index + 1
		if (field.length > 0) {
			fields.push(field)
			continue
		...
		if (fields.length === 0) {
			return Result.err("git worktree porcelain contains an empty record")
		}
		records.push(fields)
		fields = []
	}
	if (fieldStart !== worktreeList.length || fields.length > 0) {
		return Result.err("git worktree porcelain is not terminated by an empty NUL field")
	}
	...
	for (const record of records) {
		...
		for (const field of record) {
			const decoded = decodeGitField(field)
			if (!decoded.ok) return decoded
			decodedFields.push(decoded.value)
		}

		const [worktreeField, ...metadataFields] = decodedFields
		if (!worktreeField?.startsWith("worktree ") || worktreeField.length === "worktree ".length) {
			return Result.err("git worktree porcelain record does not start with a worktree path")
		}
		let headFields = 0
		let checkoutFields = 0
		let bareFields = 0
		for (const field of metadataFields) {
			if (/^HEAD [0-9a-f]+$/.test(field)) headFields += 1
			if ((field.startsWith("branch ") && field.length > "branch ".length) || field === "detached") {
				checkoutFields += 1
			}
			if (field === "bare") bareFields += 1
			const isRecognized =
				/^HEAD [0-9a-f]+$/.test(field) ||
				(field.startsWith("branch ") && field.length > "branch ".length) ||
				field === "bare" ||
				field === "detached" ||
				field === "locked" ||
			...
			if (!isRecognized) {
				return Result.err(`git worktree porcelain contains an unrecognized field: ${field}`)
			}
		}
		const hasValidBareMetadata = bareFields === 1 && headFields === 0 && checkoutFields === 0
		const hasValidWorktreeMetadata = bareFields === 0 && headFields === 1 && checkoutFields === 1
		if (!hasValidBareMetadata && !hasValidWorktreeMetadata) {
			return Result.err("git worktree porcelain contains ambiguous record metadata")
		}
		worktreePaths.push(worktreeField.slice("worktree ".length))
	}

	return Result.ok(worktreePaths)
}

async function canonicalizePossiblyMissingPath(candidatePath: string): Promise<Result<string, string>> {
	...
	while (true) {
		try {
			...
			return Result.ok(path.resolve(canonicalAncestor, ...missingSegments))
		} catch (error) {
			if (!isFilesystemError(error, "ENOENT") && !isFilesystemError(error, "ENOTDIR")) {
				return Result.err(
					`failed to canonicalize ${candidatePath}: ${error instanceof Error ? error.message : String(error)}`,
				)
			}

			const parent = path.dirname(existingAncestor)
			if (parent === existingAncestor) {
				return Result.err(`could not find an existing ancestor for ${candidatePath}`)
			}
...
async function isWorktreeRegistered(
	worktreeList: Uint8Array,
	worktreePath: string,
): Promise<Result<boolean, string>> {
	const parsedPaths = parseGitWorktreePaths(worktreeList)
	if (!parsedPaths.ok) return parsedPaths

	const canonicalTarget = await canonicalizePossiblyMissingPath(worktreePath)
	if (!canonicalTarget.ok) return canonicalTarget
	const targetKey = path.normalize(canonicalTarget.value).normalize("NFC").toLowerCase()

	for (const registeredPath of parsedPaths.value) {
		const canonicalRegisteredPath = await canonicalizePossiblyMissingPath(registeredPath)
		if (!canonicalRegisteredPath.ok) return canonicalRegisteredPath
		const registeredKey = path.normalize(canonicalRegisteredPath.value).normalize("NFC").toLowerCase()
		if (registeredKey === targetKey) return Result.ok(true)
	}

	return Result.ok(false)
}

async function processPendingWorktreeDelete(
	options: ProcessPendingWorktreeDeleteOptions,
): Promise<WorktreeDeleteOutcome> {
	const pendingDelete = getPendingDelete(options.database, options.sessionID)
	if (!pendingDelete) return { status: "none" }

	const gitFn = options.gitFn ?? git
	const gitRawFn = options.gitRawFn ?? gitRaw
	const removeWorktreeFn = options.removeWorktreeFn ?? removeWorktree
	const pathExistsFn = options.pathExistsFn ?? pathExists
	const clearDeleteStateFn = options.clearDeleteStateFn ?? clearDeleteState
	const retainPendingDelete = (reason: string): WorktreeDeleteOutcome => {
		options.log.warn(`[worktree] ${reason}; pending delete retained for retry`)
		return { status: "retained", reason }
	}
	const clearPendingState = (status: "removed" | "reconciled"): WorktreeDeleteOutcome => {
		try {
			clearDeleteStateFn(options.database, options.sessionID)
		} catch (error) {
			return retainPendingDelete(
				`worktree state cleanup failed: ${error instanceof Error ? error.message : String(error)}`,
			)
		}
	...
	let beforeSnapshotFn: BeforeSnapshot | undefined
	try {
		beforeSnapshotFn = await options.prepareDeleteFn()
	} catch (error) {
		return retainPendingDelete(
			`Pre-delete configuration failed: ${error instanceof Error ? error.message : String(error)}`,
		)
	}

	let worktreePathExists: boolean
	try {
		worktreePathExists = await pathExistsFn(pendingDelete.path)
	} catch (error) {
		return retainPendingDelete(
			`worktree path verification failed: ${error instanceof Error ? error.message : String(error)}`,
		)
	}

	if (!worktreePathExists) {
		const worktreeList = await gitRawFn(["worktree", "list", "--porcelain", "-z"], options.repoRoot)
		if (!worktreeList.ok) {
			return retainPendingDelete(`worktree registration verification failed: ${worktreeList.error}`)
		}
		const registration = await isWorktreeRegistered(worktreeList.value, pendingDelete.path)
		if (!registration.ok) {
			return retainPendingDelete(`worktree registration verification failed: ${registration.error}`)
		}
		if (registration.value) {
			return retainPendingDelete("worktree path is missing but remains registered with Git")
		}

		return clearPendingState("reconciled")
	}

	try {
		await beforeSnapshotFn?.(pendingDelete.path)
	} catch (error) {
		return retainPendingDelete(
			`Pre-delete preparation failed: ${error instanceof Error ? error.message : String(error)}`,
		)
	}

	const addResult = await gitFn(["add", "-A"], pendingDelete.path)
	if (!addResult.ok) return retainPendingDelete(`git add failed: ${addResult.error}`)

	const commitResult = await gitFn(
		["commit", "-m", "chore(worktree): session snapshot", "--allow-empty"],
		pendingDelete.path,
	)
	if (!commitResult.ok) return retainPendingDelete(`git commit failed: ${commitResult.error}`)

	const statusResult = await gitFn(["status", "--porcelain"], pendingDelete.path)
	if (!statusResult.ok) {
		return retainPendingDelete(`worktree cleanliness check failed: ${statusResult.error}`)
	}
	if (statusResult.value.trim()) {
		return retainPendingDelete("worktree remained dirty after the snapshot commit")
	}

	const removeResult = await removeWorktreeFn(options.repoRoot, pendingDelete.path)
	if (!removeResult.ok) {
		return retainPendingDelete(`worktree removal failed: ${removeResult.error}`)
	}

	return clearPendingState("removed")
}
...
/** Run post-create hooks without rolling back an otherwise usable worktree. */
async function runBestEffortHooks(cwd: string, commands: string[], log: Logger): Promise<void> {
...
/** Run safety-critical pre-delete hooks and stop at the first failure. */
async function runStrictHooks(cwd: string, commands: string[], log: Logger): Promise<void> {
	for (const command of commands) {
		log.info(`[worktree] Running hook: ${command}`)

		let result: ReturnType<typeof Bun.spawnSync>
		try {
			result = Bun.spawnSync(["bash", "-c", command], {
				cwd,
				stdout: "inherit",
				stderr: "pipe",
			})
		...
		throw new WorktreeError(
			`Hook failed with exit ${result.exitCode}: ${command}${stderr ? `\n${stderr}` : ""}`,
			"preDelete hook",
		)
	}
}
...
/**
 * Load creation configuration on a best-effort basis.
 * Missing files are created with defaults; invalid or unreadable files are logged
 * and replaced in memory with defaults so an already-created worktree remains usable.
 */
...
type ReadConfigFile = (configPath: string) => Promise<string>

function isFilesystemError(error: unknown, code: string): boolean {
	return error instanceof Error && "code" in error && error.code === code
}

/** Load deletion policy without converting invalid or unreadable config into defaults. */
async function loadWorktreeConfigForDelete(
	directory: string,
	readConfigFile: ReadConfigFile = (configPath) => readFile(configPath, "utf8"),
	...
	try {
		...
		if (isFilesystemError(error, "ENOENT")) return worktreeConfigSchema.parse({})
	...
	if (parseErrors.length > 0) {
		throw new WorktreeError(
			`Invalid JSONC in ${configPath} at offset ${parseErrors[0].offset}`,
			"preDelete config",
		)
	}
	...
	try {
		config = worktreeConfigSchema.parse(parsed)
	} catch (error) {
		throw new WorktreeError(
			`Invalid schema in ${configPath}: ${error instanceof Error ? error.message : String(error)}`,
			"preDelete config",
			error,
		)
	}

	if (config.worktreePath) config.worktreePath = resolveHomePath(config.worktreePath)
	return config
}
	...
	return {
		tool: {
			worktree_create: tool({
				...
				async execute(args, toolCtx) {
					...
					// Post-create hooks are intentionally best-effort: the worktree remains usable.
					if (worktreeConfig.hooks.postCreate.length > 0) {
						await runBestEffortHooks(worktreePath, worktreeConfig.hooks.postCreate, log)
					}
			...
			worktree_delete: tool({
				...
				async execute(_args, toolCtx) {
					...
					setPendingDelete(
						database,
						{ sessionId: toolCtx.sessionID, branch: session.branch, path: session.path },
						client,
					)
		...
		event: async ({ event }: { event: Event }): Promise<void> => {
			...
			await processPendingWorktreeDelete({
				...
				repoRoot: directory,
				log,
				gitFn: dependencies.gitFn,
				gitRawFn: dependencies.gitRawFn,
				removeWorktreeFn: dependencies.removeWorktreeFn,
				prepareDeleteFn: async () => {
					const config = await loadWorktreeConfigForDelete(
						directory,
						dependencies.readConfigFileFn,
					)
					if (config.hooks.preDelete.length === 0) return undefined
