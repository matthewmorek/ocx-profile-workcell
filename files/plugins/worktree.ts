...
function decodeGitField(field: Uint8Array): Result<string, string> {
  try {
    ...
    return Result.err(
      "git worktree porcelain contains a path that is not valid UTF-8",
    );
  }
}

function parseGitWorktreePaths(
  ...
  if (fieldStart !== worktreeList.length || fields.length > 0) {
    return Result.err(
      "git worktree porcelain is not terminated by an empty NUL field",
    );
  }
  ...
  for (const record of records) {
    ...
    if (
      !worktreeField?.startsWith("worktree ") ||
      worktreeField.length === "worktree ".length
    ) {
      return Result.err(
        "git worktree porcelain record does not start with a worktree path",
      );
    }
    let headFields = 0;
    ...
    for (const field of metadataFields) {
      ...
      if (!isRecognized) {
        return Result.err(
          `git worktree porcelain contains an unrecognized field: ${field}`,
        );
      }
    }
    const hasValidBareMetadata =
      bareFields === 1 && headFields === 0 && checkoutFields === 0;
    const hasValidWorktreeMetadata =
      bareFields === 0 && headFields === 1 && checkoutFields === 1;
    if (!hasValidBareMetadata && !hasValidWorktreeMetadata) {
      return Result.err(
        "git worktree porcelain contains ambiguous record metadata",
      );
    }
  ...
  while (true) {
    try {
      ...
      if (parent === existingAncestor) {
        return Result.err(
          `could not find an existing ancestor for ${candidatePath}`,
        );
      }
...
async function isWorktreeRegistered(
  ...
  const targetKey = path
    .normalize(canonicalTarget.value)
    .normalize("NFC")
    .toLowerCase();

  for (const registeredPath of parsedPaths.value) {
    const canonicalRegisteredPath =
      await canonicalizePossiblyMissingPath(registeredPath);
    if (!canonicalRegisteredPath.ok) return canonicalRegisteredPath;
    const registeredKey = path
      .normalize(canonicalRegisteredPath.value)
      .normalize("NFC")
      .toLowerCase();
...
async function processPendingWorktreeDelete(
  ...
  const clearPendingState = (
    status: "removed" | "reconciled",
  ): WorktreeDeleteOutcome => {
    try {
  ...
  if (!worktreePathExists) {
    const worktreeList = await gitRawFn(
      ["worktree", "list", "--porcelain", "-z"],
      options.repoRoot,
    );
    if (!worktreeList.ok) {
      return retainPendingDelete(
        `worktree registration verification failed: ${worktreeList.error}`,
      );
    }
    const registration = await isWorktreeRegistered(
      worktreeList.value,
      pendingDelete.path,
    );
    if (!registration.ok) {
      return retainPendingDelete(
        `worktree registration verification failed: ${registration.error}`,
      );
    }
    if (registration.value) {
      return retainPendingDelete(
        "worktree path is missing but remains registered with Git",
      );
    }
  ...
  if (!addResult.ok)
    return retainPendingDelete(`git add failed: ${addResult.error}`);
  ...
  if (!commitResult.ok)
    return retainPendingDelete(`git commit failed: ${commitResult.error}`);

  const statusResult = await gitFn(
    ["status", "--porcelain"],
    pendingDelete.path,
  );
  if (!statusResult.ok) {
    return retainPendingDelete(
      `worktree cleanliness check failed: ${statusResult.error}`,
    );
  }
  if (statusResult.value.trim()) {
    return retainPendingDelete(
      "worktree remained dirty after the snapshot commit",
    );
  }

  const removeResult = await removeWorktreeFn(
    options.repoRoot,
    pendingDelete.path,
  );
  if (!removeResult.ok) {
    return retainPendingDelete(
      `worktree removal failed: ${removeResult.error}`,
    );
  }
...
async function runBestEffortHooks(
  cwd: string,
  commands: string[],
  log: Logger,
): Promise<void> {
